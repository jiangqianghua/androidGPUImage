...
import com.jqh.gpuimagelib.camera.MediaGPUImageFilter;
...
public class GPUScreenRender implements GLSurfaceView.GLRender, SurfaceTexture.OnFrameAvailableListener {
    ...
    private GPUScreenFboRender screenFboRender;
    ...
    public GPUScreenRender(Context context) {
        this.context = context;
        screenFboRender = new GPUScreenFboRender(context);
...
//        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, cameraTextureId);
        ...
        screenFboRender.onChange(width, height);
        screenFboRender.onDraw(fboTextureId);
...
//        });
    ...
    private GPUScreenRender.OnSurfaceCreateListener onSurfaceCreateListener;

    public void setOnSurfaceCreateListener(GPUScreenRender.OnSurfaceCreateListener onSurfaceCreateListener) {
...
//        filter.setIsMedia(true);
        ...
        screenFboRender.addFilter(filter);
    }

    public void addTexture(BaseTexture baseTexture) {
        if (baseTexture != null){
            screenFboRender.addTexture(baseTexture);
        }
    }

    public void removeTexture(String key) {
        screenFboRender.removeTexture(key);
    }

    public void updateTexture(String id, float left, float top, float scale) {
        screenFboRender.updateTexture(id, left, top, scale);
    }
